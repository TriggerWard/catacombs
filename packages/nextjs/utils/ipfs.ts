export async function pinWardenMetadataToIPFS(
  metadata: { name: string; description: string; nillionUserKey: string },
  fileName: string,
) {
...
export async function pinCryptMetadataToIPFS(
  metadata: { fileIpfsCid: string; triggerText: string; iv: Uint8Array; fileType?: string; wardenUserKey: string },
  fileName: string,
) {
  const res = await fetch("https://api.pinata.cloud/pinning/pinJSONToIPFS", {
    method: "POST",
