...
import "./common/CommonTestBase.sol";
import "./optimistic-oracle-v3/OptimisticOracleV3Fixture.sol";
...
contract CommonOptimisticOracleV3Test is CommonTestBase {
    ...
    function _mockOracleResolved(address oracle, OracleRequest memory oracleRequest, bool assertionTruthful) internal {
        ...
        returns (bytes32)
    {
        ...
        return optimisticOracleV3.assertTruth(
            trueClaimAssertion,
    ...
    {
        vm.prank(TestAddress.account1);
        return assertingCaller.assertTruth(
            trueClaimAssertion,
            callbackRecipient,
            escalationManager,
            defaultLiveness,
    ...
    function _allocateBondAndAssertTruth(address asserter, bytes memory claim) public returns (bytes32 assertionId) {
        ...
        defaultCurrency.approve(
            address(optimisticOracleV3), optimisticOracleV3.getMinimumBond(address(defaultCurrency))
        );
        assertionId = optimisticOracleV3.assertTruthWithDefaults(claim, asserter);
        vm.stopPrank();
    }

    function _expectAssertionResolvedCallback(address callbackRecipient, bytes32 assertionId, bool assertedTruthfully)
        internal
    {
        vm.expectCall(
            callbackRecipient,
    ...
    function _expectAssertionDisputedCallback(address callbackRecipient, bytes32 assertionId) internal {
        vm.expectCall(
            callbackRecipient,
            abi.encodeWithSelector(
                OptimisticOracleV3CallbackRecipientInterface.assertionDisputedCallback.selector, assertionId
            )
        );
    }
    ...
    function _defaultCheckBalancesAfterSettle(bool disputed, bool resolvedTruethful, bool payOracleFee) internal {
        ...
        assertEq(
            defaultCurrency.balanceOf(TestAddress.account1), balancesBeforeSettle.asserter + expectedAsserterPayout
        );
        assertEq(
            defaultCurrency.balanceOf(TestAddress.account2), balancesBeforeSettle.disputer + expectedDisputerPayout
        );
